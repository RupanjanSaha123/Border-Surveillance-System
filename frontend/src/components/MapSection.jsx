...
// ── Drone icon ─────────────────────────────────────────────────────────────────
const getDroneIcon = (color = '#8B8F74') => L.divIcon({
  html: `<svg width="34" height="34" viewBox="-17 -17 34 34" xmlns="http://www.w3.org/2000/svg" style="overflow:visible">
    <circle r="6" fill="#243026" stroke="${color}" stroke-width="1.6"/>
    <line x1="-14" y1="0" x2="-6" y2="0" stroke="${color}" stroke-width="1.5"/>
    <line x1="6"   y1="0" x2="14" y2="0" stroke="${color}" stroke-width="1.5"/>
    <line x1="0" y1="-14" x2="0" y2="-6" stroke="${color}" stroke-width="1.5"/>
    <line x1="0" y1="6"   x2="0" y2="14" stroke="${color}" stroke-width="1.5"/>
    <circle cx="-14" cy="0"  r="3.2" fill="none" stroke="${color}" stroke-width="1" opacity="0.75"/>
    <circle cx="14"  cy="0"  r="3.2" fill="none" stroke="${color}" stroke-width="1" opacity="0.75"/>
    <circle cx="0"  cy="-14" r="3.2" fill="none" stroke="${color}" stroke-width="1" opacity="0.75"/>
    <circle cx="0"  cy="14"  r="3.2" fill="none" stroke="${color}" stroke-width="1" opacity="0.75"/>
    <circle r="2.5" fill="${color}"/>
    <circle r="20" fill="none" stroke="${color}" stroke-width="0.8" opacity="0.35" stroke-dasharray="5 4">
...
const DEFAULT_DRONE_ICON = getDroneIcon('#8B8F74');
const REAL_DRONE_ICON = getDroneIcon('#4ade80');
...
const ThreatMarker = ({ mapCenter }) => {
  ...
  useEffect(() => {
    ...
    const { lat, lng, sector } = mapCenter;
  ...
  return (
    <div style={{ ...s, width:'100%', height:'100%', position:'relative' }}>
      ...
      <MapContainer center={[32.50, 76.55]} zoom={11} style={{ width:'100%', height:'100%' }} zoomControl={false}>
        ...
        {/* Animated Default Drone */}
        <Marker position={pos} icon={DEFAULT_DRONE_ICON}/>
